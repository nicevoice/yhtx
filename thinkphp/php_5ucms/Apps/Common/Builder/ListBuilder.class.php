...
class ListBuilder extends Controller{
    ...
    public function addTopButton($type, $attribute = null){
        switch($type){
            ...
            case 'resume': //添加启用按钮(禁用的反操作)
                ...
                $my_attribute['href']  = U(MODULE_NAME.'/'.CONTROLLER_NAME.'/setstate', array('state' => 'resume', 'model' => $my_attribute['data-model']));
            ...
            case 'forbid': //添加禁用按钮(启用的反操作)
                ...
                $my_attribute['href']  = U(MODULE_NAME.'/'.CONTROLLER_NAME.'/setstate', array('state' => 'forbid', 'model' => $my_attribute['data-model']));
            ...
            case 'recycle': //添加回收按钮(还原的反操作)
                ...
                $my_attribute['data-model'] = $attribute['model'] ? : CONTROLLER_NAME; //要操作的数据模型
                $my_attribute['href']  = U(MODULE_NAME.'/'.CONTROLLER_NAME.'/setstate', array('state' => 'recycle', 'model' => $my_attribute['data-model']));
            ...
            case 'restore': //添加还原按钮(回收的反操作)
                ...
                $my_attribute['class'] = 'btn btn-success ajax-post confirm';
                $my_attribute['data-model'] = $attribute['model'] ? : CONTROLLER_NAME; //要操作的数据模型
                $my_attribute['href']  = U(MODULE_NAME.'/'.CONTROLLER_NAME.'/setstate', array('state' => 'restore', 'model' => $my_attribute['data-model']));
            ...
            case 'delete': //添加删除按钮(我没有反操作，删除了就没有了，就真的找不回来了)
                ...
                $my_attribute['class'] = 'btn btn-danger ajax-post confirm';
                $my_attribute['data-model'] = $attribute['model'] ? : CONTROLLER_NAME; //要操作的数据模型
                $my_attribute['href']  = U(MODULE_NAME.'/'.CONTROLLER_NAME.'/setstate', array('state' => 'delete', 'model' => $my_attribute['data-model']));
    ...
    public function addRightButton($type, $attribute = null){
        switch($type){
            ...
            case 'forbid': //改变记录状态按钮，会更具数据当前的状态自动选择应该显示启用/禁用
                ...
                $my_attribute['0']['href']  = U(MODULE_NAME.'/'.CONTROLLER_NAME.'/setstate', array('state' => 'resume', 'ids' => '__data_id__', 'model' => $my_attribute['data-model']));
                $my_attribute['1']['title'] = '禁用';
                $my_attribute['1']['class'] = 'label label-warning ajax-get confirm';
                $my_attribute['1']['href']  = U(MODULE_NAME.'/'.CONTROLLER_NAME.'/setstate', array('state' => 'forbid', 'ids' => '__data_id__', 'model' => $my_attribute['data-model']));
            ...
            case 'hide': //改变记录状态按钮，会更具数据当前的状态自动选择应该显示隐藏/显示
                ...
                $my_attribute['2']['href']  = U(MODULE_NAME.'/'.CONTROLLER_NAME.'/setstate', array('state' => 'show', 'ids' => '__data_id__', 'model' => $my_attribute['data-model']));
                $my_attribute['1']['title'] = '隐藏';
                $my_attribute['1']['class'] = 'label label-info ajax-get confirm';
                $my_attribute['1']['href']  = U(MODULE_NAME.'/'.CONTROLLER_NAME.'/setstate', array('state' => 'hide', 'ids' => '__data_id__', 'model' => $my_attribute['data-model']));
            ...
            case 'recycle':
                ...
                $my_attribute['data-model'] = $attribute['model'] ? : CONTROLLER_NAME; //要操作的数据模型
                $my_attribute['href'] = U(MODULE_NAME.'/'.CONTROLLER_NAME.'/setstate', array('state' => 'recycle', 'ids' => '__data_id__', 'model' => $my_attribute['data-model']));
            ...
            case 'restore':
                ...
                $my_attribute['data-model'] = $attribute['model'] ? : CONTROLLER_NAME; //要操作的数据模型
                $my_attribute['href'] = U(MODULE_NAME.'/'.CONTROLLER_NAME.'/setstate', array('state' => 'restore', 'ids' => '__data_id__', 'model' => $my_attribute['data-model']));
            ...
            case 'delete':
                ...
                $my_attribute['class'] = 'label label-danger ajax-get confirm';
                $my_attribute['data-model'] = $attribute['model'] ? : CONTROLLER_NAME; //要操作的数据模型
                $my_attribute['href'] = U(MODULE_NAME.'/'.CONTROLLER_NAME.'/setstate', array('state' => 'delete', 'ids' => '__data_id__', 'model' => $my_attribute['data-model']));
    ...
    public function display(){
            ...
            if($this->_right_button_list){
                foreach($this->_right_button_list as $right_button){
                    ...
                    if($right_button['type'] === 'forbid' || $right_button['type'] === 'hide'){
                        $right_button = $right_button[$data['state']];
                    }
		...
		parent:: show(ChageATurl($this -> fetch($this->_template)));  //自动修改资源路径150901 
    }
