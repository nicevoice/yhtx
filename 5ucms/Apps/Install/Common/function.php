...
function check_dirfile(){
	$items = array(
		array('dir',  '可写', 'success', './uploadfile/'),
		array('dir',  '可写', 'success', './Apps/Runtime'),
		array('file', '可写', 'success', './Apps/Common/Conf/db.php'),
...
function write_config($config, $auth){
	if(is_array($config)){
		...
		//写入应用配置文件
		if(!IS_WRITE){
			return '由于您的环境不可写，请复制下面的配置文件内容覆盖到相关的配置文件，然后再登录后台。<p>'.realpath(APP_PATH).'/Common/Conf/db.php</p>
...
function register_administrator($db, $prefix, $admin, $auth){
	...
	$sql = "INSERT INTO `".$prefix."admin` VALUES " .
		   "('1', '[username]', '[password]', '[email]', 'setting,channel,sitelink,label,upload,create,diypage,admin,tags', '0', '0', '0', 'gif/jpg/png/bmp/jpeg/mp3/wma/rm/rmvb/ra/asf/avi/wmv/swf/rar/exe/zip/doc/xls', '1024', '17098715006');";

	$password = newmd5($admin['password']);
	$sql = str_replace(
		array('[username]', '[password]', '[email]'),
		array($admin['username'], $password, $admin['email']),
		$sql);
	//执行sql
	$db->execute($sql);
