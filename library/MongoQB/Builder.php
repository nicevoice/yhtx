 ...
 * @codeCoverageIgnore
 */
class MongoQB_Exception extends Exception {}

class MongoQB_Builder
{
	/**
	 * Config file data
	 *
	...
	public function __construct(array $config, $connect = true)
	{
		if ( ! class_exists('Mongo')) {
			...
			throw new MongoQB_Exception('The MongoDB PECL extension has not been
	...
	public function setConfig($config = array(), $connect = true)
	{
		if (is_array($config)) {
			...
			throw new MongoQB_Exception('No config variables passed');
		}
	...
	{
		if (empty($dsn)) {
			throw new MongoQB_Exception('To switch MongoDB databases, a
		...
		try {
			...
			throw new MongoQB_Exception('Unable to switch Mongo Databases: ' .
	...
	*/
	public function dropDb($database = '')
	{
		if (empty($database)) {
		   ...
		   throw new MongoQB_Exception('Failed to drop MongoDB database because
			name is empty');
			...
			// @codeCoverageIgnoreStart
				...
				throw new MongoQB_Exception('Unable to drop Mongo database `' .
	...
	{
		if (empty($database)) {
			throw new MongoQB_Exception('Failed to drop MongoDB collection because
		...
		if (empty($collection)) {
			...
			// @codeCoverageIgnoreStart
				...
				throw new MongoQB_Exception('Unable to drop Mongo collection `' .
	...
	*/
	...
	{
		if (empty($collection)) {
			throw new MongoQB_Exception('In order to retrieve documents from
		...
		while ($cursor->hasNext()) {
			...
			// @codeCoverageIgnoreStart
				...
				throw new MongoQB_Exception($Exception->getMessage());
				// @codeCoverageIgnoreEnd
			}
		}
	...
	*/
	...
	{
		if (empty($collection)) {
			throw new MongoQB_Exception('In order to retrieve a count of
	...
	{
		if (empty($collection)) {
			throw new MongoQB_Exception('No Mongo collection selected to insert
			 into');
		}

		if (count($insert) === 0 OR ! is_array($insert)) {
			throw new MongoQB_Exception('Nothing to insert into Mongo collection
		...
		// @codeCoverageIgnoreStart
			...
			throw new MongoQB_Exception('Insert of data into MongoDB failed: ' .
			 $Exception->getMessage());
			// @codeCoverageIgnoreEnd
		}
	}
	...
	{
		if (empty($collection)) {
			throw new MongoQB_Exception('No Mongo collection selected to
			 update');
		}

		if (count($this->updates) === 0) {
			throw new MongoQB_Exception('Nothing to update in Mongo collection or
		...
		// @codeCoverageIgnoreStart
			...
			throw new MongoQB_Exception('Update of data into MongoDB failed: ' .
			 $Exception->getMessage());
			// @codeCoverageIgnoreEnd
		}
	}
	...
	{
		if (empty($collection)) {
			throw new MongoQB_Exception('No Mongo collection selected to delete
			 from');
		}
		...
		// @codeCoverageIgnoreStart
			...
			throw new MongoQB_Exception('Delete of data into MongoDB failed: ' .
			 $Exception->getMessage());
			// @codeCoverageIgnoreEnd
		}
	}
	...
	{
		if (empty($collection)) {
			throw new MongoQB_Exception('No Mongo collection selected to delete from');
		}
	...
	public function distinct($collection='',$field=''){
		if (empty($collection)) {
			throw new MongoQB_Exception('No Mongo collection selected to disc from');
		}
		if (empty($field)) {
			throw new MongoQB_Exception('No Mongo collection field selected to disc for');
		}
		try {
			...
			throw new MongoQB_Exception('distinct of data into MongoDB failed: ' .$Exception->getMessage());
		}
	}
	...
	public function command($query = array())
	{
		...
		// @codeCoverageIgnoreStart
			...
			throw new MongoQB_Exception('MongoDB command failed to execute: ' .
			 $Exception->getMessage());
			// @codeCoverageIgnoreEnd
		}
	}
	...
	{
		if (empty($collection)) {
			throw new MongoQB_Exception('No MongoDB collection specified to add
			 index to');
		}

		if (empty($fields) OR ! is_array($fields)) {
			throw new MongoQB_Exception('Index could not be added to MongoDB
		...
		// @codeCoverageIgnoreStart
			...
			throw new MongoQB_Exception('An error occurred when trying to add an
	...
	{
		if (empty($collection)) {
			throw new MongoQB_Exception('No Mongo collection specified to remove
			 index from');
		}

		if (empty($keys) OR ! is_array($keys)) {
			throw new MongoQB_Exception('Index could not be removed from MongoDB
			 Collection because no keys were specified');
		}
	   ...
	   if ($this->_dbhandle->{$collection}->deleteIndex($keys)) {
			...
			// @codeCoverageIgnoreStart
			throw new MongoQB_Exception('An error occurred when trying to remove
	...
	private function _connect()
	{
		...
		// @codeCoverageIgnoreStart
				...
				throw new MongoQB_Exception('Unable to connect to MongoDB: ' .
				 $Exception->getMessage());
				// @codeCoverageIgnoreEnd
		}
	}
	...
	private function _connectionString()
	{
		...
		if (empty($this->_dsn)) {
			throw new MongoQB_Exception('The DSN is empty');
		}
		...
		if ( ! isset($parts['path']) OR str_replace('/', '', $parts['path']) === '') {
			throw new MongoQB_Exception('The database name must be set in the DSN string');
		}
